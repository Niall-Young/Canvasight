...
import {
  comparableNode,
  comparableNodeSemantic,
  changedFromBase,
  documentEdgeConstraintViolations,
  documentObjectWriters,
  documentsContentEqual,
  documentsViewStateEqual,
  edgeIncidentConflict,
  itemMap,
  mergeAtomicItems,
  pageContentChanged,
...
function deprecatedAssetRoleAdvisories(args) {
  ...
  const inspectNode = (node, nodePath) => {
    ...
    if (Object.prototype.hasOwnProperty.call(node, "role")) entries.push(`${nodePath}.role`);
    if (isObject(node.data) && Object.prototype.hasOwnProperty.call(node.data, "role")) entries.push(`${nodePath}.data.role`);
  };
  if (Array.isArray(args?.nodes)) args.nodes.forEach((node, index) => inspectNode(node, `nodes[${index}]`));
  if (Array.isArray(args?.pages)) {
    args.pages.forEach((page, pageIndex) => {
      if (Array.isArray(page?.nodes)) page.nodes.forEach((node, nodeIndex) => inspectNode(node, `pages[${pageIndex}].nodes[${nodeIndex}]`));
    });
  }
  if (Array.isArray(args?.operations)) {
    args.operations.forEach((operation, index) => {
      if (operation?.op === "add-node") inspectNode(operation.node, `operations[${index}].node`);
      if (operation?.op === "update-node" && isObject(operation.changes)) inspectNode(operation.changes, `operations[${index}].changes`);
    });
  }
...
function normalizeScatterViewState(value) {
  ...
  return {
    collapsedGroupIds: Array.isArray(source.collapsedGroupIds)
      ? [...new Set(source.collapsedGroupIds.filter((id) => typeof id === "string" && id.trim()).map((id) => id.trim()))]
      : []
  };
}
...
function normalizeScatterPage(value, index, fallback) {
  ...
  for (const node of nodes) {
    if (node.type === "group" || node.parentId === undefined) continue;
    if (!groupIds.has(node.parentId)) throw new HttpError(400, `Node parentId must reference a Group node: ${node.id}`);
  }
...
function normalizeScatterDocument(value, projectPath) {
  ...
  const version = value.version === 2 || pages.some((page) => page.nodes.some((node) => node.type !== "task" || node.parentId)) ? 2 : 1;
  return {
    ...documentFields,
    version,
...
function documentWithCurrentNavigation(currentDocument, localDocument, projectPath, preserveLocalViewState = false) {
  ...
  const pages = localDocument.pages.map((page) => {
    ...
    return currentPage
      ? { ...page, viewport: currentPage.viewport, viewState: preserveLocalViewState ? page.viewState : currentPage.viewState }
      : page;
  });
...
function applyGraphAutoLayout(nodes, edges, layout, positionAxesByNodeId) {
  ...
  const macroEdges = edges.flatMap((edge) => {
    ...
    macroPairs.add(pair);
    return [{ id: `macro:${edge.id}`, source, target }];
  });
  ...
  const laidOut = nodes.map((node) => {
    ...
    const autoPosition = node.type !== "group" && node.parentId && groupIds.has(node.parentId)
      ? internalPositions.get(node.id)
      : macroPositions.get(node.id);
...
function graphRequestNodeInputs(args) {
  ...
  if (Array.isArray(args?.operations)) {
    args.operations.forEach((operation) => {
      if (operation?.op === "add-node" && isObject(operation.node)) inputs.push(operation.node);
    });
  }
  return inputs;
}
...
async function prepareGraphTemplateAssets(args, templates, projectPath) {
  ...
  if (selectedTemplateIds.size === 0) {
    return { templates, pendingAssetPaths: new Set(), entries: [] };
  }
  ...
  const pendingAssetPaths = new Set();
  const entries = [];
  ...
  for (const template of templates) {
    ...
    for (const value of template.attachments) {
      const attachment = normalizeAttachment(value);
      const sourceCandidate = typeof value?.storedPath === "string" && value.storedPath.trim()
        ? value.storedPath.trim()
        : typeof value?.relativePath === "string" && value.relativePath.trim()
          ? path.resolve(canvasightHome(), value.relativePath.trim())
          : "";
      const sourcePath = path.resolve(sourceCandidate || ".");
      if (!sourceCandidate || !isPathInside(sourcePath, templateAssetsRoot)) {
        throw new HttpError(400, `Template attachment is not a Canvasight template asset: ${attachment.originalName}`, "invalid_template_asset_path");
      }
      let stat;
      try {
        stat = await fsp.lstat(sourcePath);
      } catch {
        throw new HttpError(400, `Template attachment is unavailable: ${attachment.originalName}`, "template_asset_unavailable");
      }
      if (!stat.isFile() || stat.isSymbolicLink()) {
        throw new HttpError(400, `Template attachment must be a regular file: ${attachment.originalName}`, "invalid_template_asset_file");
      }
      const originalName = safeFileName(attachment.originalName);
      const uniqueName = `${Date.now()}-${crypto.randomBytes(4).toString("hex")}-${originalName}`;
...
async function commitPreparedGraphAssets(entries, referencedPaths) {
  ...
  try {
    for (const entry of selected) {
      ...
      if (!stat.isFile() || stat.isSymbolicLink()) {
        throw new HttpError(400, `Template attachment must remain a regular file: ${path.basename(entry.sourcePath)}`, "invalid_template_asset_file");
      }
      await fsp.mkdir(path.dirname(entry.storedPath), { recursive: true });
      const temporaryPath = `${entry.storedPath}.${process.pid}.${crypto.randomBytes(4).toString("hex")}.tmp`;
      try {
...
function referencedPreparedAssetPaths(pages, pendingAssetPaths) {
  return new Set(
    pages
      ...
      .map((node) => path.resolve(node.data?.asset?.storedPath || "."))
      .filter((storedPath) => pendingAssetPaths.has(storedPath))
  );
}

function normalizeManagedGraphAsset(value, projectPath, fieldPath, pendingAssetPaths = new Set()) {
  if (!isObject(value)) throw new HttpError(400, `${fieldPath} must reference a managed project asset`);
  const assetsRoot = path.resolve(scatterAssetsDir(projectPath));
  const relativePath = typeof value.relativePath === "string" ? value.relativePath.trim() : "";
  const candidatePath = typeof value.storedPath === "string" && value.storedPath.trim()
    ? value.storedPath.trim()
    : relativePath
  ...
  if (!candidatePath || !isPathInside(storedPath, assetsRoot)) {
    throw new HttpError(400, `${fieldPath} must reference a managed project asset`);
  }
  ...
  if (!pendingAssetPaths.has(storedPath)) {
    ...
    try {
      ...
      throw new HttpError(400, `${fieldPath} must reference an available managed project asset`);
    }
    if (!stat.isFile() || stat.isSymbolicLink()) throw new HttpError(400, `${fieldPath} must reference a regular managed project asset`);
  }
  return asset;
}

function normalizeGraphNode(value, index, layout, usedNodeIds, projectPath, templates = [], reusedTemplates = [], templateExpansions = [], pendingAssetPaths = new Set()) {
  ...
  if (type === "group" && (value.parentId !== undefined || data.parentId !== undefined)) {
    throw new HttpError(400, `nodes[${index}].parentId cannot nest a Group`);
  }
  ...
  const requestedAttachments = Array.isArray(value.attachments)
    ? value.attachments
    : Array.isArray(data.attachments)
      ? data.attachments
      : [];
  if (type === "task" && requestedAttachments.length > 0) {
    throw new HttpError(400, `nodes[${index}].attachments cannot create inline Task attachments; create managed Asset nodes instead`, "task_inline_attachments_forbidden");
  }
  if (template) {
  ...
  const parentId = typeof value.parentId === "string" && value.parentId.trim()
    ...
    : typeof data.parentId === "string" && data.parentId.trim()
      ...
      : "";
  if (type === "asset") {
    const assetInput = isObject(value.asset) ? value.asset : data.asset;
    return {
      ...base,
      ...(parentId ? { parentId } : {}),
      data: {
        ...
        asset: normalizeManagedGraphAsset(assetInput, projectPath, `nodes[${index}].asset`, pendingAssetPaths)
      }
    };
  }
...
function derivedGraphId(prefix, sourceId, usedIds) {
  ...
  let id = `${prefix}-${safeSource}`;
  let suffix = 2;
  while (usedIds.has(id)) id = `${prefix}-${safeSource}-${suffix++}`;
  usedIds.add(id);
  return id;
}
...
function expandTemplateAssets(nodes, edges, templateExpansions, projectPath, pendingAssetPaths, language = "zh") {
  ...
  for (const expansion of templateExpansions) {
    expansion.attachments.forEach((attachment, index) => {
      const assetNodeId = derivedGraphId("template-asset", `${expansion.taskNodeId}-${index + 1}`, usedNodeIds);
      const edgeId = derivedGraphId("template-asset-edge", `${expansion.taskNodeId}-${index + 1}`, usedEdgeIds);
      const asset = normalizeManagedGraphAsset(attachment, projectPath, `template attachment for ${expansion.taskNodeId}`, pendingAssetPaths);
      expandedNodes.push({
        id: assetNodeId,
        type: "asset",
        position: { x: 0, y: 0 },
...
function buildScatterPageFromGraph(value, index, args, projectPath, templates = [], reusedTemplates = [], projectGuidanceNodes = [], pendingAssetPaths = new Set()) {
  const page = isObject(value) ? value : {};
  const now = nowIso();
  ...
  const baseNodes = rawNodeInputs.map((node, nodeIndex) =>
    normalizeGraphNode(node, nodeIndex, layout, usedNodeIds, projectPath, templates, reusedTemplates, templateExpansions, pendingAssetPaths)
  );
...
function mergeGraphNodeChanges(node, changes, projectPath) {
  ...
  const requestedTaskAttachments = attachments !== undefined ? attachments : dataChanges.attachments;
  if (node.type === "task" && requestedTaskAttachments !== undefined) {
  ...
  const parentChanges = node.type === "group" || parentId === undefined
    ? {}
  ...
  const typeDataChanges = node.type === "asset"
    ? {
        ...
        ...(role !== undefined ? { role: normalizeAssetRole(role) } : {}),
        ...(asset !== undefined ? { asset: normalizeManagedGraphAsset(asset, projectPath, `node ${node.id} asset`) } : {})
      }
    : node.type === "group"
      ? { ...(typeof description === "string" ? { description } : {}) }
      : {
...
function applyMergeOperations(existingPage, args, projectPath, templates, reusedTemplates, pendingAssetPaths = new Set()) {
    ...
    try {
      ...
      if (operation.op === "update-node") {
        ...
        if (Object.prototype.hasOwnProperty.call(operation.changes, "type") && operation.changes.type !== page.nodes[nodeIndex].type) {
          violations.push(graphViolation("immutable_node_type", `${opPath}.changes.type`, "Node types cannot be changed in place.", "Remove the node and add a new node with the desired type."));
          return;
        }
      ...
      if (operation.op === "promote-attachment") {
        if (typeof operation.nodeId !== "string" || nodeIndex < 0 || page.nodes[nodeIndex].type !== "task") {
          violations.push(graphViolation("task_node_not_found", `${opPath}.nodeId`, "promote-attachment requires an existing Task node.", "Re-read graph context and use a Task node id."));
          return;
        }
        ...
        if (page.nodes.some((node) => node.id === assetNodeId)) {
          violations.push(graphViolation("duplicate_node_id", `${opPath}.assetNodeId`, `Duplicate node id: ${assetNodeId}`, "Choose a unique Asset node id."));
          return;
        }
        if (page.edges.some((edge) => edge.id === promotedEdgeId)) {
          violations.push(graphViolation("duplicate_edge_id", `${opPath}.edgeId`, `Duplicate edge id: ${promotedEdgeId}`, "Choose a unique Edge id."));
          return;
        }
        const task = page.nodes[nodeIndex];
        const attachment = task.data.attachments.find((item) => item.id === attachmentId);
        if (!attachment) {
          violations.push(graphViolation("legacy_attachment_not_found", `${opPath}.attachmentId`, "The legacy Task attachment does not exist.", "Re-read graph context and use a returned legacyAttachments id."));
          return;
        }
        const asset = normalizeManagedGraphAsset(attachment, projectPath, `${opPath}.attachmentId`, pendingAssetPaths);
        ...
        page.edges.push({
          ...
          source: task.id,
          target: assetNodeId,
          label: typeof operation.edgeLabel === "string" && operation.edgeLabel.trim()
            ? operation.edgeLabel.trim()
            : args?.language === "en" ? "Attachment" : "附件"
        });
      ...
      violations.push(graphViolation("unsupported_operation", `${opPath}.op`, `Unsupported operation: ${operation.op}`, "Use add/update/remove node or edge operations, promote-attachment, or relayout-page."));
    } catch (error) {
...
function ensureMergedSoftwareProductGuidance(page, args, projectPath, templates, reusedTemplates, pendingAssetPaths = new Set()) {
...
function graphNodeBounds(node, nodes = []) {
  ...
  const x = toNumber(node?.position?.x, 0) + toNumber(parent?.position?.x, 0);
...
function validateGraphStructure(page, layout = "horizontal") {
  ...
  page.edges.forEach((edge, index) => {
    ...
    if (nodeById.get(edge.source)?.type === "group" || nodeById.get(edge.target)?.type === "group") {
      violations.push(graphViolation("group_edge_forbidden", `edges[${index}]`, "Edges cannot connect to Group nodes.", "Use parentId for membership and connect Task or Asset nodes for semantic relationships."));
    }
  ...
  page.nodes.forEach((node, index) => {
    if (node.type === "group") {
      if (node.parentId) violations.push(graphViolation("nested_group_forbidden", `nodes[${index}].parentId`, "Groups cannot belong to another Group.", "Remove parentId from the Group."));
      return;
    }
    if (!node.parentId) return;
    const parent = nodeById.get(node.parentId);
    if (!parent) violations.push(graphViolation("group_parent_not_found", `nodes[${index}].parentId`, `Group parent does not exist: ${node.parentId}`, "Use an existing Group id or remove parentId."));
    else if (parent.type !== "group") violations.push(graphViolation("parent_not_group", `nodes[${index}].parentId`, `parentId does not reference a Group: ${node.parentId}`, "Use a Group id or remove parentId."));
  });
...
async function writeScatterGraph(projectPath, args) {
  return withProjectWriteLock(projectPath, async () => {
    ...
    let preparedTemplateAssets = { templates: availableTemplates, pendingAssetPaths: new Set(), entries: [] };
    ...
    if (mode === "merge-active-page") {
      ...
      const validation = validateGraphCandidate(candidatePage, args, projectPath, {
        preferences,
        requiredCoverageNodeIds: [...combinedSummary.addedNodeIds, ...combinedSummary.updatedNodeIds].filter((nodeId) =>
          !generatedGuidanceNodeIds.has(nodeId) && candidatePage.nodes.find((node) => node.id === nodeId)?.type === "task"
        )
      });
      ...
      const incomingPages = graphPageInputs(args).map((page, index) =>
        buildScatterPageFromGraph(page, index, args, projectPath, templates, reusedTemplates, projectGuidanceNodes, preparedTemplateAssets.pendingAssetPaths)
      );
      for (let index = 0; index < incomingPages.length; index += 1) {
        ...
        const validation = validateGraphCandidate(incomingPages[index], args, projectPath, {
          preferences,
          requiredCoverageNodeIds: incomingPages[index].nodes
            .filter((node) => node.type === "task" && !generatedGuidanceNodeIds.has(node.id))
            .map((node) => node.id)
        });
        if (!validation.passed) return validationFailure(currentRevision, validation.violations, validation.advisories);
        validationAdvisories.push(...validation.advisories);
    ...
    let document;
    try {
      document = await writeScatterDocument(projectPath, {
      ...
      await Promise.all(committedGraphAssetPaths.map((storedPath) => fsp.rm(storedPath, { force: true }).catch(() => undefined)));
      throw error;
    }
...
function summarizeGraphContextNode(node, projectPath) {
  ...
  return {
    ...
    legacyAttachments: Array.isArray(node.data?.attachments)
      ? node.data.attachments.map((value) => {
          const attachment = normalizeAttachment(value);
          return {
            id: attachment.id,
