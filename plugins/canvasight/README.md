...
使用完整的三个反引号围栏，可选填写语言名。上下围栏各自既可以使用三个英文半角反引号，也可以使用三个中文全角反引号；编辑中切换输入法也能识别，保存时不会改写原文：
...
在 `v0.4.22` 中不需要打开额外开关：完整的三个半角或全角反引号围栏、显式 `@plugin` / `$skill` 和合法 `http` / `https` URL 会在节点正文中自动获得语义样式；输入 `$` 还会继续打开 Skill 选择器。
...
Use a complete triple-backtick fence, optionally with a language name. Each opening or closing fence may use either three ASCII backticks or three full-width backticks, so switching input methods while editing is supported without rewriting the original text:
...
No extra toggle is required in `v0.4.22`: complete three-character ASCII or full-width backtick fences, explicit `@plugin` / `$skill` text, and valid `http` / `https` URLs receive their semantic presentation automatically. Typing `$` also continues to open the Skill picker.
