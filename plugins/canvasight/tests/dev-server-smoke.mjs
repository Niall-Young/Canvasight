...
async function main() {
  try {
    ...
    const [videoAttachment] = await fetchJson(`${origin}/api/sessions/local/attachments`, {
      method: "POST",
      body: JSON.stringify({
        projectPath,
        files: [{
    ...
    assert.equal(videoAttachment.mime, "video/mp4", "dev uploads must infer video MIME");
