...
3. Use `open_canvasight` when widget rendering is unavailable or the user needs the in-app browser/dev page. This fallback must claim the current thread before Run so the daemon can target direct `turn/start`; if native app-server delivery fails, it falls back to `await_canvasight_run`.
4. If a browser fallback URL is returned, open the full `browserUrl` / `url`, including query parameters, in Codex's in-app Browser/sidebar. Do not navigate only to the origin.
5. If an old browser page is already open and the user moved to a new Codex thread, call `claim_canvasight_thread` before Run. Successful Runs should arrive through daemon `turn/start`; use `await_canvasight_run` only when the response or UI says the payload was queued.
6. Use `list_canvasight_recent_projects` and `open_canvasight_recent_project` when the user wants the last Canvasight project from a new Codex thread and no widget/browser page is currently usable.
7. Call `close_canvasight` only when the specific session is no longer needed. It does not stop the project-level daemon.
