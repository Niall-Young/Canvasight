...
6. If an old browser page is already open and the user moved to a new Codex thread, call `claim_canvasight_thread` before Run. Successful Runs should arrive through daemon `turn/start`; use `await_canvasight_run` only when the response or UI says the payload was queued.
