...
4. If Canvasight MCP tools are unavailable and you must open the already running dev page with generic browser control, read the current shell `CODEX_THREAD_ID` and the current workspace/project path, then open `http://127.0.0.1:5173/?threadId=<current-thread-id>&projectPath=<absolute-project-path>`. URL-encode the project path. Do not open bare `http://127.0.0.1:5173/`, and do not open a thread-only URL, for a thread that expects project-scoped Run delivery.
