...
1. Reproduce the normal path with the current `CODEX_THREAD_ID`: call `open_canvasight`, capture its `sessionId`, then call `await_canvasight_widget_ready` with that `sessionId` and the same `threadId`.
2. Treat only `status: "ready"` plus `reactMounted: true` as widget readiness. For `timeout`, `failed`, or incomplete readiness, record `sessionId`, `threadId`, `projectPath`, `stage`, `reactMounted`, `error`, and `reportedAt` before changing anything.
