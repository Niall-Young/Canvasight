...
- `canvasight_mcp_transport_closed` / MCP error `Transport closed`: the current thread can see Canvasight tool names but the live MCP transport is closed. Reload or open a new Codex thread; do not treat browser fallback as native Run recovery.
...
1. Confirm the canvas was opened through `open_canvasight` native widget output, not a bare `http://127.0.0.1:5173/` dev page or browser fallback URL. If `open_canvasight` was not visible, call `tool_search` for `canvasight open_canvasight render_canvasight_canvas_widget` and then open the native widget. If the native tool is visible but returns `Transport closed`, report `canvasight_mcp_transport_closed` and require reload/new thread.
