...
3. If the user already clicked Run but no Codex turn appeared, call `await_canvasight_run`. If `await_canvasight_run` is not callable, use `tool_search` for Canvasight tools first. If it is callable but fails with `Transport closed`, report `canvasight_mcp_transport_closed`: the fallback queue cannot be consumed from this stale thread. Inspect the 0.1.44+ MCP lifecycle log when available, then reload/new-thread with current Canvasight tools.
...
Native open must have received the active task's explicit `CODEX_THREAD_ID` as `threadId`; otherwise Chat / Plan / Goal preflight must fail before bridge send. Do not use a missing `hostCapabilities.message` declaration alone to disable Run after the MCP App handshake succeeds. Call `sendMessage` and treat only its resolved Promise as sent.
