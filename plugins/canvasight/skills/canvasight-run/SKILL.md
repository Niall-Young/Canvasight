...
Native open must have received the active task's explicit `CODEX_THREAD_ID` as `threadId` and passed `await_canvasight_widget_ready` with `status: "ready"` plus `reactMounted: true`; otherwise the canvas is not verified ready for Run. Do not use a missing `hostCapabilities.message` declaration alone to disable Run after the MCP App handshake succeeds. Call `sendMessage` and treat only its resolved Promise as sent.
