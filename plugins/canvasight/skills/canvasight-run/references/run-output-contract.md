...
Default plugin Run clicks should come from native widget delivery. Browser URL and bare dev Run clicks should be treated as queued fallback payloads unless a real Codex turn appears. A `sent` result from an isolated app-server process is not sufficient evidence that the live Codex Desktop thread received the Markdown.
