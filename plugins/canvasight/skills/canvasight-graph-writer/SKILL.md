...
5. Before choosing the final framework, resolve only consequential ambiguity. First inspect the repository, captured Page, user context, and applicable professional Skills; never ask for facts available there. Call `ask_canvasight_framework_questions` only when at least two plausible answers would materially change content mode, framework dimensions, target scope, key relationships, write behavior, or required coverage. Ask the highest-priority one to three questions together, with two or three concrete options per question plus custom input; choose `single` or `multiple` from the decision semantics. Do not ask about node count, routine wording, or decoration. After calling the tool, stop the graph-write turn and wait for its visible user-message response. If the tool is unavailable in an older task or inline UI cannot render, ask the same questions as concise ordinary text; never open Canvasight, invoke another visualization surface, or guess a consequential answer.
