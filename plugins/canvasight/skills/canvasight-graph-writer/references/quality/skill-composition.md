...
In `skill-led` mode, do not add default business, domain, maturity, or guidance content merely to satisfy Canvasight's canonical contracts. Still cover every Task Node created or updated by the write, state every compound Task responsibility, describe every edge between covered Task Nodes, pass revision and atomic-write checks, and preserve horizontal left-to-right topology. Asset Nodes may serve as evidence/input but are not required responsibility coverage; Groups never satisfy coverage.
...
A node-level assignment is Task-only: it means that a Skill should execute only for that Task Node's visible responsibility. Asset and Group nodes never receive Skill assignments. A Task assignment is represented twice in one write:
...
Every assignment must reference a final candidate Task Node and exactly match a `$skill-name` token in its body. Multiple Skills are allowed when each has a distinct responsibility-specific role. Never add hidden Page fields or remove existing visible Task assignments merely because the preference is off.
