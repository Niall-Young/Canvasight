...
The plugin opens a canvas workspace for arranging task nodes, attachments, and prompt flows. The web app is served by a project-level local daemon that outlives thread-local MCP shim processes. Normal plugin use renders Canvasight inside a Codex native widget through `open_canvasight`; callers must read the active task's `CODEX_THREAD_ID` and pass it as `threadId` because MCP child processes may not inherit it and Chat / Plan / Goal preflight requires an explicit target. The widget directly hosts the built app and calls the daemon for APIs instead of iframe-loading localhost. Widget CSP must include the current daemon's exact origin so direct widget app fetches are not blocked by the host. Native open tool results must not publicly expose daemon `127.0.0.1` URLs; daemon URL, origin, and token belong only in `_meta.widgetData` for the widget shell. Running a node sends Markdown for that node plus downstream children to the current Codex thread through the widget host bridge after native Chat / Plan / Goal preflight. The widget bridge may use standard MCP Apps `ui/message` or the Codex/OpenAI compatibility `window.openai.sendFollowUpMessage`; both count as native widget host bridge transports, not browser fallback. A successful MCP App handshake makes `sendMessage` callable even if the host omits the advisory `hostCapabilities.message` declaration; only the send Promise result determines sent/failed. Browser URLs and bare dev pages are fallback/development surfaces: after a current-thread claim they only queue payloads for `await_canvasight_run`; they must not report app-server `turn/start` as a successful Run path.
...
- Normal plugin opening should read the active task's `CODEX_THREAD_ID`, pass it as `threadId` to `open_canvasight`, and render the Canvasight native widget so Run can send a follow-up message to that same Codex thread through the host bridge after mode preflight. If `open_canvasight` is not visible in the active tool list, use `tool_search` for `canvasight open_canvasight render_canvasight_canvas_widget` before declaring the native tool unavailable. Use `open_canvasight_browser_fallback` only as an explicit browser fallback or debugging path.
- `npm run dev` starts or reuses the project-level persistent Canvasight dev server at `http://127.0.0.1:5173/`. The command exits after the server is ready; archiving the launching Codex thread should not stop the dev server. If the recorded managed dev server `serverVersion` differs from the current package version, `npm run dev` stops the stale Vite process and starts a fresh one so old API middleware is not reused. Bare dev Run uses only the daemon's latest `claim_canvasight_thread` binding for the project or an explicit URL `threadId` claim. Since `0.1.37`, generic dev fallback resolves that thread's Codex project `cwd` and creates/opens `.scatter` in that project; `projectPath` is only an explicit override when already known. It must not show a manual project path gate during normal opening, and it must not fall back to the dev server process `CODEX_THREAD_ID`; when no claim exists, it returns `unbound_dev_session`. Since `0.1.39`, browser/dev fallback is diagnostic-only for Run delivery: claimed Runs queue for `await_canvasight_run` and must not report app-server `turn/start` as sent. Since `0.1.40`, normal native open hides daemon URLs from public tool output and exposes them only to the widget shell through `_meta.widgetData`. Since `0.1.41`, the widget bridge adapter can send through either MCP Apps `ui/message` or `window.openai.sendFollowUpMessage`, and diagnostics expose the selected `bridgeTransport` and failure reason. Since `0.1.42`, `browser_fallback_no_bridge` is treated as an opening-path error and normal recovery must lazy-load native tools with `tool_search` instead of opening localhost. Since `0.1.43`, visible Canvasight tools failing with `Transport closed` are classified as `canvasight_mcp_transport_closed`: the current Codex thread's MCP transport is stale/dead and recovery is reload/new thread, not localhost fallback. Since `0.1.44`, the MCP stdio shim logs lifecycle events to the Canvasight state directory, exits cleanly on stdin end, returns JSON-RPC errors for tool failures, and `npm run dev` / `dev:status` also detect and clear stale cross-version daemon state. Since `0.1.46`, native open requires an explicit current `threadId`, native mode preflight defaults on, transient active-rollout read races are retried, a connected MCP App attempts `sendMessage` even when `hostCapabilities.message` is not advertised, and initial `toolResponseMetadata` can bootstrap the direct app. Normal sent status still requires the bridge Promise to resolve successfully.
- Since `0.1.47`, daemon startup is single-flight across MCP shims, old daemons cannot remove state owned by a new process, stdout EPIPE closes the shim without recursive logging, and `mcp-lifecycle.log` is capped at 5 MB by default.
- Since `0.1.48`, orphan cleanup covers both source-checkout and installed-cache Canvasight daemon paths when they share the default state directory.
- Since `0.1.49`, the native widget executes its inline Vite bundle as an ES module. It stays in an explicit loading state until the bundle succeeds and shows a recovery error instead of a false `Canvasight ready` status when the bundle fails.
