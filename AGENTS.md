...
- AI graph generation combines intent, primary domain, optional secondary domain, maturity, and output topology through `frameworkManifest`. Submit coverage for the primary domain's required content. Validation failures are internal repair instructions: correct the candidate and retry, up to three rounds, before writing or reporting completion. Do not expose a raw violation list as the delivered user result.
...
- `get_canvasight_graph_context` returns the active Page's lightweight node, edge, position, Page-summary, and document-revision context before an AI update. `write_canvasight_graph` keeps legacy append/replace modes and adds revision-protected `merge-active-page` operations for minimal node and edge changes.
