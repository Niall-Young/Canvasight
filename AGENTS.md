...
`open_canvasight` tool completion means only that the MCP call prepared or returned a widget session; it is not proof that the canvas opened. Native opening succeeds only after the actual widget runtime emits a positive ready acknowledgement after React has mounted and the initial session/API health check has completed. Until that acknowledgement exists and is observed, the result is `unverified`; agents must not say the canvas is "opened", "ready", or "fixed" based on tool output, resource reads, daemon health, browser fallback, or synthetic harnesses alone.
...
- Observe the real widget ready acknowledgement, confirm the full canvas is visible, exercise at least one meaningful canvas control, and confirm a node Run reaches the same Codex task through the native host bridge.
...
- Native open requires an explicit current `threadId` and normal mode preflight. Public tool output must not expose daemon URLs or tokens; widget-only session data stays in `_meta.widgetData`. A completed tool response is provisional until the widget emits its ready acknowledgement.
