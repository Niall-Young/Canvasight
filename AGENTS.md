...
The plugin opens a canvas workspace for arranging task nodes, attachments, and prompt flows. The web app is served by a project-level local daemon that outlives thread-local MCP shim processes. Normal plugin use renders Canvasight inside a Codex native widget through `open_canvasight`; the widget directly hosts the built app and calls the daemon for APIs instead of iframe-loading localhost. Widget CSP must include the current daemon's exact origin so direct widget app fetches are not blocked by the host. Native open tool results must not publicly expose daemon `127.0.0.1` URLs; daemon URL, origin, and token belong only in `_meta.widgetData` for the widget shell. Running a node sends Markdown for that node plus downstream children to the current Codex thread through the widget host bridge after native Chat / Plan / Goal preflight. The widget bridge may use standard MCP Apps `ui/message` or the Codex/OpenAI compatibility `window.openai.sendFollowUpMessage`; both count as native widget host bridge transports, not browser fallback. Browser URLs and bare dev pages are fallback/development surfaces: after a current-thread claim they only queue payloads for `await_canvasight_run`; they must not report app-server `turn/start` as a successful Run path.
...
- `npm run test:mcp` runs the MCP smoke test, including daemon persistence across MCP process restarts and stdio lifecycle handling for newline and `Content-Length` JSON-RPC transports.
