...
The plugin opens a canvas workspace for arranging task nodes, attachments, and prompt flows. The web app is served by a project-level local daemon that outlives thread-local MCP shim processes. Normal plugin use renders Canvasight inside a Codex native widget through `open_canvasight`; the widget directly hosts the built app and calls the daemon for APIs instead of iframe-loading localhost. Widget CSP must include the current daemon's exact origin so direct widget app fetches are not blocked by the host. Running a node or flow sends Markdown to the current Codex thread through the widget host bridge. Browser URLs and bare dev pages are fallback/development surfaces: after a current-thread claim they ask the daemon to start the Codex turn, but app-server `turn/start` accepted is not proof that the live Codex Desktop thread visibly received a message unless a matching `turn/started`, `item/started`, or `turn/completed` notification confirms delivery. Unconfirmed fallback Runs remain queued for `await_canvasight_run`.
