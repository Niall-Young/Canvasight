...
- Before a Graph Writer write, use `ask_canvasight_framework_questions` only for unresolved ambiguity that would materially change content mode, framework dimensions, target scope, key relationships, write behavior, or required coverage. First inspect the repository, current Page, user context, and applicable Skills. Group at most three high-priority questions into the inline card, stop the current write turn, and after the visible answer message reacquire graph context before continuing. Do not ask about node counts, routine wording, or decoration. If the tool or inline UI is unavailable, ask concise text questions; never open Canvasight or guess a consequential answer as the fallback.
