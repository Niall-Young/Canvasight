...
- 让画布服务独立于单个 Codex thread 存活；native widget 直接发到当前 thread，浏览器 fallback 可由当前 thread claim 后领取后续 Run payload。
...
7. 点击节点或流程的 Run。
8. 点击 Run 后，native widget 会通过 host bridge 把 Markdown 作为 follow-up message 发送到当前 Codex 线程。如果使用的是浏览器 URL 或裸 dev 页面 fallback，先调用 `claim_canvasight_thread` claim 该项目；之后点击 Run 会先通过项目级 daemon 的 native `turn/start` 发送到当前线程，失败时才进入队列，再用 `await_canvasight_run` 加 `sessionId` 或 `projectPath` 接收 Run payload。
...
升级后可用 `codex plugin list` 确认 `canvasight@canvasight-local` 显示为 `0.1.29` 或更高版本。如果显示低于 `0.1.29`，旧的 MCP cache 可能还在运行旧版 server，请重新执行 `codex plugin add canvasight@canvasight-local` 并新开线程。
...
`open_canvasight` 保留为浏览器 URL fallback。它默认面向 Codex 侧边栏内置浏览器，不会直接调用系统默认浏览器；开发调试时如需外部浏览器，可显式设置 `CANVASIGHT_OPEN_EXTERNAL_BROWSER=1`。新 Codex 线程里如果旧浏览器 fallback 已经打开，先调用 `claim_canvasight_thread` claim 最近项目或指定 `projectPath`；如果需要新 URL，再调用 `list_canvasight_recent_projects` 和 `open_canvasight_recent_project`。浏览器 fallback 点击 Run 会先通过 daemon native `turn/start` 发到当前 claim 的线程；如果 app-server 不可用或显式设置 `CANVASIGHT_CODEX_NATIVE=0`，才进入 daemon 队列，随后 `await_canvasight_run` 可以按 `sessionId` 等待，也可以按 `projectPath` attach 到同一项目的 Run 队列。正常插件使用不需要手动运行 `npm run dev`。
...
`npm run dev` 会启动或复用项目级持久 dev server，默认地址是 `http://127.0.0.1:5173/`。它用于本地“运行项目”和开发预览，启动命令退出或启动它的 Codex thread 被归档后，服务仍应继续存活。裸 `5173` 页面只使用 daemon 里最新的 `claim_canvasight_thread` 项目绑定，或显式 URL `threadId` claim；不会再退回启动 dev server 时的 `CODEX_THREAD_ID`。没有绑定时 Run 返回 `unbound_dev_session`，避免错发到旧线程。默认会尝试 native app-server `turn/start` 直发到已绑定线程，失败或显式设置 `CANVASIGHT_CODEX_NATIVE=0` 时才进入队列等待 `await_canvasight_run`。需要手动停止时运行 `npm run dev:stop`。
...
AI 生成画布只是写入 `.scatter/scatter.json`，创建可编辑的 Page、节点和连线。Run 是用户在 widget 或网页里点击节点或流程后提交 Markdown payload；native widget 会直接发到当前 Codex thread，浏览器 fallback 在 claim 后也会先通过 daemon native `turn/start` 直发，失败时才放入对应项目/thread 的队列并由 `await_canvasight_run` 接管后续执行。
...
会。本地画布服务由项目级 daemon 托管，不再绑定打开它的 thread-local MCP 进程。归档旧 thread 后，从当前 thread 重新调用 `render_canvasight_canvas_widget` 即可得到当前 thread 的 widget bridge。旧浏览器 fallback tab 也可继续编辑；在新 thread 接收 fallback Run 前先调用 `claim_canvasight_thread`，后续 Run payload 会归属到新 thread 并优先通过 daemon `turn/start` 发送，失败时再等待 `await_canvasight_run` 接收。如果手动停止 daemon 或机器重启导致旧 URL 失效，再从当前 thread 重新打开最近项目即可。
...
先确认 Canvasight 是通过 `render_canvasight_canvas_widget` 打开的；native widget 成功时 Run 会作为当前 thread 的 follow-up message 出现。如果你打开的是 `open_canvasight` 返回的浏览器 URL 或 `npm run dev` 裸页面，它不会天然拥有 Codex host bridge，但 `claim_canvasight_thread` 后会通过 daemon native `turn/start` 发到当前线程。没有 claim 的裸 `5173` 页面会提示未绑定，而不是把 Run 发到启动 dev server 的旧线程。若仍无效，查看 UI 是否提示 queued，并用 `await_canvasight_run` 接收一次排查；重装后仍无效时，新开 Codex thread 或 reload，确认 `codex plugin list` 显示 `0.1.29` 或更高。
...
不会。Canvasight 不使用虚拟点击、辅助功能权限或剪贴板发送。native widget Run 通过 Codex host bridge 发送 follow-up message；浏览器 fallback Run 在 claim 后通过 daemon native `turn/start` 发送，失败时才进入 MCP daemon 队列，并通过 `await_canvasight_run` 返回给当前 Codex thread。
...
- Keep the canvas service alive outside a single Codex thread. The native widget sends directly to the current thread; browser fallbacks can be claimed by the current thread and use daemon native `turn/start` before queue fallback.
...
8. Click Run. The native widget sends the Markdown as a follow-up message to the current Codex thread through the host bridge. If you are using a browser URL or bare dev-page fallback, call `claim_canvasight_thread` for that project first; later Run clicks first try daemon native `turn/start` to the current thread, and only fall back to `await_canvasight_run` by `sessionId` or `projectPath` when direct delivery fails.
...
After upgrading, run `codex plugin list` and confirm `canvasight@canvasight-local` shows `0.1.29` or newer. If it shows a version below `0.1.29`, the old MCP cache may still be running an older server; run `codex plugin add canvasight@canvasight-local` again and open a new thread.
...
AI-generated canvas data writes `.scatter/scatter.json` to create editable Pages, nodes, and edges. Run happens after the user clicks a node or flow in the widget or web app. The native widget sends the Markdown to the current Codex thread directly; browser fallbacks use daemon native `turn/start` after claim and queue the payload for `await_canvasight_run` only when direct delivery fails.
...
Call `claim_canvasight_thread` with `projectPath` or `sessionId` from the current Codex thread so an older browser tab scopes future Runs to this thread. The next Run should direct-send through daemon `turn/start`; call `await_canvasight_run` only if the UI or returned delivery state says it was queued. Pass `sessionId` if you know it from the old URL, otherwise pass `projectPath`.

**What if Run does not appear in the current thread?**

First confirm Canvasight was opened with `render_canvasight_canvas_widget`; successful native widget Runs appear as follow-up messages in the current thread. If you opened a browser URL from `open_canvasight` or a bare `npm run dev` page, that page does not automatically have the Codex host bridge, but it should direct-send through daemon `turn/start` after `claim_canvasight_thread`. An unclaimed bare `5173` page now reports that it is unbound instead of sending to the dev server's old launch thread. If it still queues, call `await_canvasight_run` and inspect the returned reason. If reinstalling did not change behavior, open a new Codex thread or reload, and confirm `codex plugin list` shows `0.1.29` or newer.
...
No. Canvasight does not use virtual clicks, Accessibility automation, or clipboard sending. Native widget Runs use the Codex host bridge to send a follow-up message. Browser fallback Runs use daemon native `turn/start` after `claim_canvasight_thread`; if direct delivery fails, they enter the MCP daemon queue and return through `await_canvasight_run`.
