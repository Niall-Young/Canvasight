...
升级后可用 `codex plugin list` 确认 `canvasight@canvasight-local` 显示为 `0.1.41` 或更高版本。如果显示低于 `0.1.41`，旧的 MCP cache 可能还在运行旧版 server，请重新执行 `codex plugin add canvasight@canvasight-local` 并新开线程。
...
先确认 Canvasight 是通过 `open_canvasight` native widget 打开的；native widget 成功时 Run 会作为当前 thread 的 follow-up message 出现。如果你打开的是 `open_canvasight_browser_fallback` 返回的浏览器 URL 或 `npm run dev` 裸页面，它不会拥有 Codex host bridge；`claim_canvasight_thread` 后也只会排队给当前线程领取。没有 claim 的裸 `5173` 页面会提示未绑定，而不是把 Run 发到启动 dev server 的旧线程。若 UI 提示 queued，用 `await_canvasight_run` 接收一次排查；若提示 `Canvasight daemon did not start in time`，先看 `npm run dev:status` 是否为 stale，然后运行 `npm run dev` 让 managed dev server 自动重启。重装后仍无效时，新开 Codex thread 或 reload，确认 `codex plugin list` 显示 `0.1.41` 或更高。左下角诊断按钮会显示当前是否为 direct widget / fallback、`canvasightHost`、bridge 可用性、`bridgeTransport`、`bridgeReason`，以及最近一次 Run 的 `status/via/reason/error`。
...
After upgrading, run `codex plugin list` and confirm `canvasight@canvasight-local` shows `0.1.41` or newer. If it shows a version below `0.1.41`, the old MCP cache may still be running an older server; run `codex plugin add canvasight@canvasight-local` again and open a new thread.
