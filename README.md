...
Canvasight 是一个 repo-local Codex 插件。它会打开一个项目级常驻的本地网页画布，让你把任务拆成节点、连接流程、添加附件，然后把当前节点或整条流程生成 Markdown 和结构化数据发送到当前 Codex 线程；跨线程或旧网页接收时仍可用 `await_canvasight_run` 兜底。
...
如果网页已经打开，外部写入后刷新页面或重新打开项目即可看到 AI 生成的 Page。AI 写入画布只是创建可编辑节点和连线，不会自动运行节点；真正发送通过点击 Run。已绑定当前 Codex thread 时会直接发送，旧网页或跨线程接收时用 `await_canvasight_run` 兜底。
...
升级后可用 `codex plugin list` 确认 `canvasight@canvasight-local` 显示为 `0.1.17` 或更高版本。如果仍是 `0.1.0`、`0.1.1`、`0.1.2`、`0.1.3`、`0.1.4`、`0.1.5`、`0.1.6`、`0.1.7`、`0.1.8`、`0.1.9`、`0.1.10`、`0.1.11`、`0.1.12`、`0.1.13`、`0.1.14`、`0.1.15` 或 `0.1.16`，旧的 MCP cache 可能还在运行旧版 server，请重新执行 `codex plugin add canvasight@canvasight-local` 并新开线程。
...
AI 生成画布只是写入 `.scatter/scatter.json`，创建可编辑的 Page、节点和连线。Run 是用户在网页里点击节点或流程后，把 Markdown payload 发送到当前绑定的 Codex thread；无法直接发送或从旧网页接收时，再由当前调用 `await_canvasight_run` 的 Codex thread 接管。
...
不会。Canvasight 不使用虚拟点击、辅助功能权限或剪贴板发送。Run payload 通过 MCP daemon 和 Codex app-server `turn/start` 发送；无法直接发送时再通过 `await_canvasight_run` 返回给当前 Codex thread。

---

## English

Canvasight is a repo-local Codex plugin. It opens a project-level persistent local browser canvas where you can break work into task nodes, connect flows, attach files, and send generated Markdown plus structured data to the current Codex thread; `await_canvasight_run` remains the fallback for cross-thread or older browser sessions.
...
If the web app is already open, refresh the page or reopen the project to see the externally generated Page. AI-generated canvas data only creates editable nodes and edges; it does not run nodes. Sending happens when the user clicks Run: bound sessions send directly to the current Codex thread, while older or cross-thread sessions can still be received with `await_canvasight_run`.
...
After upgrading, run `codex plugin list` and confirm `canvasight@canvasight-local` shows `0.1.17` or newer. If it still shows `0.1.0`, `0.1.1`, `0.1.2`, `0.1.3`, `0.1.4`, `0.1.5`, `0.1.6`, `0.1.7`, `0.1.8`, `0.1.9`, `0.1.10`, `0.1.11`, `0.1.12`, `0.1.13`, `0.1.14`, `0.1.15`, or `0.1.16`, the old MCP cache may still be running an older server; run `codex plugin add canvasight@canvasight-local` again and open a new thread.
...
AI-generated canvas data writes `.scatter/scatter.json` to create editable Pages, nodes, and edges. Run happens after the user clicks a node or flow in the web app, then the Markdown payload is sent to the bound current Codex thread. If direct delivery is not available, the Codex thread calling `await_canvasight_run` receives it.
...
No. Canvasight does not use virtual clicks, Accessibility automation, or clipboard sending. Run payloads go through the MCP daemon and Codex app-server `turn/start`; queued fallbacks return through `await_canvasight_run`.
