...
- 让画布服务独立于单个 Codex thread 存活；native widget 直接发到当前 thread，浏览器 fallback 可由当前 thread claim 后把后续 Run payload 放入可领取队列。
...
7. 点击节点或流程的 Run。
8. 点击 Run 后，native widget 会通过 host bridge 把 Markdown 作为 follow-up message 发送到当前 Codex 线程。如果使用的是浏览器 URL 或裸 dev 页面 fallback，先调用 `claim_canvasight_thread` claim 该项目；之后点击 Run 会把 payload 放入当前 thread 可领取的队列，再用 `await_canvasight_run` 加 `sessionId` 或 `projectPath` 接收 Run payload。
...
`npm run dev` 会启动或复用项目级持久 dev server，默认地址是 `http://127.0.0.1:5173/`。它用于本地“运行项目”和开发预览，启动命令退出或启动它的 Codex thread 被归档后，服务仍应继续存活。`0.1.35` 起，如果记录的 managed dev server `serverVersion` 低于当前 package 版本，`npm run dev` 会自动停止旧 Vite 进程并重启，避免旧 API 中间件继续处理 Run。`npm run dev:status` 会显示 `running`、`stopped` 或 `stale`。裸 `5173` 页面只使用 daemon 里最新的 `claim_canvasight_thread` 项目绑定，或显式 URL `threadId` + `projectPath` claim；不会再退回启动 dev server 时的 `CODEX_THREAD_ID`。没有绑定时 Run 返回 `unbound_dev_session`，避免错发到旧线程。`0.1.36` 起，通用 dev fallback 支持 `?threadId=<id>&projectPath=<absolute-path>`，避免只带 threadId 时打开默认 Canvasight repo 项目。有绑定时 Run 默认进入队列等待 `await_canvasight_run`；只有 app-server 收到匹配确认事件时才会显示为 sent。需要手动停止时运行 `npm run dev:stop`。
...
AI 生成画布只是写入 `.scatter/scatter.json`，创建可编辑的 Page、节点和连线。Run 是用户在 widget 或网页里点击节点或流程后提交 Markdown payload；native widget 会直接发到当前 Codex thread，浏览器 fallback 在 claim 后放入对应项目/thread 的队列并由 `await_canvasight_run` 接管后续执行。
...
不会。Canvasight 不使用虚拟点击、辅助功能权限或剪贴板发送。native widget Run 通过 Codex host bridge 发送 follow-up message；浏览器 fallback Run 在 claim 后进入 MCP daemon 队列，并通过 `await_canvasight_run` 返回给当前 Codex thread。
...
- Keep the canvas service alive outside a single Codex thread. The native widget sends directly to the current thread; browser fallbacks can be claimed by the current thread and place Run payloads in the current thread's queue.
...
8. Click Run. The native widget sends the Markdown as a follow-up message to the current Codex thread through the host bridge. If you are using a browser URL or bare dev-page fallback, call `claim_canvasight_thread` for that project first; later Run clicks queue the payload for `await_canvasight_run` by `sessionId` or `projectPath`.
...
Call `claim_canvasight_thread` with `projectPath` or `sessionId` from the current Codex thread so an older browser tab scopes future Runs to this thread. The next Run enters the current thread's Canvasight queue; call `await_canvasight_run` to receive it. Pass `sessionId` if you know it from the old URL, otherwise pass `projectPath`.
...
No. Canvasight does not use virtual clicks, Accessibility automation, or clipboard sending. Native widget Runs use the Codex host bridge to send a follow-up message. Browser fallback Runs enter the MCP daemon queue after `claim_canvasight_thread` and return through `await_canvasight_run`.
