...
- Task/Asset/Group Run 始终通过 Chat 把对应范围发送到当前 Codex 任务。
...
- 产品/UI 设计探索默认可生成三张独立参考图；用户选择一张，将它连接到前端实现 Task 后从该 Asset 运行，Canvasight 只携带所选图片及其下游任务，让 AI 按这张图还原界面。
...
- Task/Asset/Group Run always uses Chat to send its corresponding scope to the current Codex task.
...
- Product/UI design exploration can generate three independent references by default. After choosing one, connect it to the frontend implementation Task and Run from that Asset; Canvasight sends only the selected image and its downstream work so AI can reproduce that visual.
