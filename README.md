...
7. 点击节点或流程的 Run。
8. 点击 Run 后，native widget 会通过 host bridge 把 Markdown 作为 follow-up message 发送到当前 Codex 线程。如果使用的是浏览器 URL 或裸 dev 页面 fallback，先调用 `claim_canvasight_thread` claim 该项目；之后点击 Run 会进入该项目队列，再用 `await_canvasight_run` 加 `sessionId` 或 `projectPath` 在当前线程接收 Run payload。
...
升级后可用 `codex plugin list` 确认 `canvasight@canvasight-local` 显示为 `0.1.26` 或更高版本。如果显示低于 `0.1.26`，旧的 MCP cache 可能还在运行旧版 server，请重新执行 `codex plugin add canvasight@canvasight-local` 并新开线程。
...
`open_canvasight` 保留为浏览器 URL fallback。它默认面向 Codex 侧边栏内置浏览器，不会直接调用系统默认浏览器；开发调试时如需外部浏览器，可显式设置 `CANVASIGHT_OPEN_EXTERNAL_BROWSER=1`。新 Codex 线程里如果旧浏览器 fallback 已经打开，先调用 `claim_canvasight_thread` claim 最近项目或指定 `projectPath`；如果需要新 URL，再调用 `list_canvasight_recent_projects` 和 `open_canvasight_recent_project`。浏览器 fallback 点击 Run 会进入 daemon 队列；`await_canvasight_run` 可以按 `sessionId` 等待，也可以按 `projectPath` attach 到同一项目的 Run 队列。正常插件使用不需要手动运行 `npm run dev`。
...
AI 生成画布只是写入 `.scatter/scatter.json`，创建可编辑的 Page、节点和连线。Run 是用户在 widget 或网页里点击节点或流程后提交 Markdown payload；native widget 会直接发到当前 Codex thread，浏览器 fallback 才放入对应项目/thread 的队列并由 `await_canvasight_run` 接管后续执行。
...
会。本地画布服务由项目级 daemon 托管，不再绑定打开它的 thread-local MCP 进程。归档旧 thread 后，从当前 thread 重新调用 `render_canvasight_canvas_widget` 即可得到当前 thread 的 widget bridge。旧浏览器 fallback tab 也可继续编辑；在新 thread 接收 fallback Run 前先调用 `claim_canvasight_thread`，后续 Run payload 会归属到新 thread 并等待 `await_canvasight_run` 接收。如果手动停止 daemon 或机器重启导致旧 URL 失效，再从当前 thread 重新打开最近项目即可。
...
不会。Canvasight 不使用虚拟点击、辅助功能权限或剪贴板发送。native widget Run 通过 Codex host bridge 发送 follow-up message；浏览器 fallback Run 进入 MCP daemon 队列，并通过 `await_canvasight_run` 返回给当前 Codex thread。
...
- Keep the canvas service alive outside a single Codex thread. The native widget sends directly to the current thread; browser fallbacks can be claimed by the current thread before receiving queued Runs.
...
8. Click Run. The native widget sends the Markdown as a follow-up message to the current Codex thread through the host bridge. If you are using a browser URL or bare dev-page fallback, call `claim_canvasight_thread` for that project first; later Run clicks enter that project's queue, and the current thread receives the payload with `await_canvasight_run` by `sessionId` or `projectPath`.
...
After upgrading, run `codex plugin list` and confirm `canvasight@canvasight-local` shows `0.1.26` or newer. If it shows a version below `0.1.26`, the old MCP cache may still be running an older server; run `codex plugin add canvasight@canvasight-local` again and open a new thread.
...
AI-generated canvas data writes `.scatter/scatter.json` to create editable Pages, nodes, and edges. Run happens after the user clicks a node or flow in the widget or web app. The native widget sends the Markdown to the current Codex thread directly; browser fallbacks queue the payload for `await_canvasight_run`.
...
No. Canvasight does not use virtual clicks, Accessibility automation, or clipboard sending. Native widget Runs use the Codex host bridge to send a follow-up message. Browser fallback Runs enter the MCP daemon queue and return through `await_canvasight_run`.
