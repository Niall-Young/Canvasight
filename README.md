...
升级后可用 `codex plugin list` 确认 `canvasight@canvasight-local` 显示为 `0.1.43` 或更高版本。如果显示低于 `0.1.43`，旧的 MCP cache 可能还在运行旧版 server，请重新执行 `codex plugin add canvasight@canvasight-local` 并新开线程。
...
After upgrading, run `codex plugin list` and confirm `canvasight@canvasight-local` shows `0.1.43` or newer. If it shows a version below `0.1.43`, the old MCP cache may still be running an older server; run `codex plugin add canvasight@canvasight-local` again and open a new thread.
