...
用户重启 Codex 后，新任务 `019f4abf-4e09-7a61-b97a-25f2c45353c9` 已正确加载 `0.2.0+codex.20260710062412`。`open_canvasight` 创建 session `session-mreke5jq-23e0fa0d`，真实 widget 从 Starting 进入 `Connecting Canvasight session...`，但 `await_canvasight_widget_ready` 在 15 秒后 timeout。

该状态只能由 `widgetBridge.handleToolResult` 成功解析 session metadata 后产生，因此 React/bridge 已执行；旧 ready telemetry 的默认 `reactMounted:false` 不能证明 React 未挂载。故障位于 metadata 之后、ready ACK 之前。代码检查确认 native widget 的 `getSession()` 仍直接 fetch daemon localhost，且资源读取可能早于 daemon 精确端口写入 CSP。现有 smoke 又通过手工派发 `canvasight:app-ready` 绕过了真实 `getSession -> widget-ready` 链路。

主线程已实施第二阶段修复，详见 `agent-reports/resolved/20260710-1450-development-solution-native-widget-api-proxy.md`：native JSON API 全部改走 app-only MCP proxy；资源读取等待 daemon 后再返回精确 CSP origin；测试禁止 native localhost fetch 并覆盖 proxy session GET/ready POST。新版本 `0.2.0+codex.20260710064916` 已安装。真实宿主复验前 issue 继续保持 assigned。
