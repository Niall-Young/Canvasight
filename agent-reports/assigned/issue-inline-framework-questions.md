...
solution_report: agent-reports/resolved/solution-inline-framework-flat-form-contrast.md
---
...
- [x] 原生宿主缺失证据已如实记录
- [x] 消息宿主提供外层容器时，inline 表单本身不再绘制第二层卡片外壳
- [x] 暗色与亮色主题下，未选中、选中、聚焦三种选项状态均通过自动化与截图验证
- [ ] 重启后的 Codex Desktop 新任务完成 exact 0.4.27 inline 提交与 fullscreen 回归

## 当前状态

blocked：0.4.27 已完成代码修复、自动化、截图、版本化安装和快照一致性检查；等待重启 Codex Desktop 后在新任务完成真实 inline 提交与 fullscreen 回归。

## 处理结果

移除 inline 表单的第二层外卡，同时严格保留已验收的 Scatter Figma Kit 选择控件：中性 input surface、selected connecting border、无蓝色整行填充。0.4.27 已安装；真实宿主复验待重启。

## 修改文件

- 见 `agent-reports/resolved/solution-inline-framework-flat-form-contrast.md`。
...
当前任务仍持有旧插件资源快照；在重启后 exact 0.4.27 真实宿主复验前，不得宣称 native inline 已完全验收。
