...
updated_at: 2026-07-18T08:27:44Z
depends_on:
...
blocked。commit/install 门禁与 exact 安装仍有效；首次 0×0 失败已归因到非前台 native host presentation，新受控前台实例已完成完整 ready。仍等待真实 Refresh、meaningful control、A→B→A、同任务 Run 与 late metadata 验收。不得 push、tag、创建 Release 或推进 `stable`。
...
已提交并安装 exact 0.4.31 候选，但真实原生门禁失败，当前禁止发布。0.4.29 与 0.4.30 同样保持禁止发布；三个版本均无远端 tag、Release、workflow 或 stable mutation。
