...
- `issue-codex-react-185-sidebar-recovery` | assigned | critical | Test Supervisor Agent | Codex 历史 Canvasight 侧栏恢复触发 React #185 | `agent-reports/assigned/issue-codex-react-185-sidebar-recovery.md` | 6 | 2026-07-26T06:05:44Z
- `issue-widget-viewport-recovery-save-count` | assigned | medium | Test Supervisor Agent | Widget viewport recovery 多一次 document save | `agent-reports/assigned/issue-widget-viewport-recovery-save-count.md` | 1 | 2026-07-26T05:51:42Z
...
- `issue-native-widget-attachment-relative-url` | assigned | high | Development Agent | 原生 Widget 附件图片相对地址导致破图 | `agent-reports/assigned/issue-native-widget-attachment-relative-url.md` | 5 | 2026-07-14T02:33:45Z
...
- `agent-reports/assigned/20260710-1506-development-issue-native-open-attempt-refactor.md` - reused-container binding fix, 30s timeout fallback, automated gates, and version `0.4.4+codex.20260712135359` reinstall completed; remains critical assigned/unverified until a restarted Codex host passes first-open/rebind/control/Run/late-metadata acceptance.
...
- `agent-reports/resolved/solution-windows-cli-daemon-state-cleanup-0-4-35.md` - records the Windows SIGTERM root cause and replacement-safe cleanup solution.
- `agent-reports/resolved/integration-summary-test-supervisor-0-4-35-native-acceptance.md` - independently passes exact 0.4.35 strict ready, task return, 50% Refresh, same-task Run and post-Run stability.
- `agent-reports/resolved/integration-summary-windows-daemon-stop-candidate-0-4-35.md` - integrates ownership-safe stop completion, replacement-daemon regression, full local matrix, exact install and passed native gate (v3).
...
- `agent-reports/resolved/20260710-2213-skill-expert-solution-chat-only-run-contract.md` - made the Canvasight Run skill Chat-only and defined legacy Plan/Goal node and queued payload data as Chat-normalized compatibility input.
- `agent-reports/resolved/20260710-2211-customer-support-solution-chat-only-node-run-docs.md` - updated the bilingual README for Chat-only node Run and removed obsolete Plan / Goal mode-selection, blocking, and recovery guidance.
- `agent-reports/resolved/20260710-2200-integration-summary-native-mode-truthfulness.md` - corrected the Plan/Goal false-positive contract: all paths now block without instance-bound Desktop mode acknowledgement, while Chat delivery remains supported; version 0.3.6 is installed.
- `agent-reports/resolved/20260710-2148-skill-expert-solution-native-mode-contract.md` - removed the invalid independent app-server Plan/Goal claim from the Run skill: current host exposes only follow-up delivery, so Plan/Goal block before send until an instance-bound host acknowledgement exists.
- `agent-reports/resolved/20260710-2146-test-solution-native-mode-control-gates.md` - verified Plan/Goal widget blocking prevents independent app-server and bridge sends, while Chat bridge delivery remains covered.
- `agent-reports/resolved/20260710-2146-development-solution-native-mode-confirmation.md` - host mode confirmation is unavailable, so Plan/Goal now block across widget and browser/await paths without independent app-server false positives; Chat behavior remains unchanged.
- `agent-reports/resolved/20260710-2142-customer-support-solution-native-plan-ack-docs.md` - documented the strict Desktop native mode acknowledgement gate: bridge delivery alone cannot claim or send Plan / Goal; current host behavior blocks those modes when acknowledgement is absent.
- `agent-reports/resolved/20260710-2125-integration-summary-desktop-runtime-compatibility.md` - integrated Desktop runtime selection, strict no-PATH-fallback behavior, diagnostics, docs, MCP coverage, version 0.3.5 installation, and the remaining real native-host gate.
- `agent-reports/resolved/20260710-2120-test-solution-desktop-runtime-selection.md` - covers explicit, Codex.app, ChatGPT.app, PATH-only selection and failed-Desktop no-PATH-fallback gates.
- `agent-reports/resolved/20260710-2108-test-issue-desktop-runtime-selection.md` - resolved runtime selection and native Plan/Goal fallback regression gap.
- `agent-reports/resolved/20260710-2110-development-solution-desktop-runtime-compatibility.md` - implemented fixed runtime resolution, diagnostics, and strict no-fallback semantics; candidate-order and failure-boundary smoke is resolved in the Test Supervisor report.
- `agent-reports/resolved/20260710-2108-customer-support-solution-desktop-runtime-compatibility-docs.md` - documented bilingual Desktop runtime preference, unavailable-runtime versus incompatible-archive recovery, and strict Plan/Goal send blocking; real native-host acceptance remains required.
