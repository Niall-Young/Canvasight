...
`agent-reports/assigned/issue-refresh-base-document-fingerprint-order.md`（读取版本：2）
...
- exact install 与真实 native-host acceptance 待 Codex Desktop 重启后完成。

## 后续风险

- 方案不迁移 order-sensitive fingerprint；这是有意保留的兼容边界。任何 canonical fingerprint 改造必须另立数据格式与 revision-history 迁移设计。
- 自动化只能证明协议和浏览器/widget runtime 行为，不能替代 exact 0.4.32 的 Codex native fullscreen ready、真实 Refresh、控件和 same-task Run 验收。
- issue 在实现、验证和主线程回写前仍应保持 assigned；本 solution report 不代表发布问题已关闭。
