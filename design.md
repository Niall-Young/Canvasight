...
Run is a submit action, not a Markdown preview action. A successful node Run should report sent only after the native widget host bridge `sendMessage` promise succeeds. Browser/dev fallback surfaces may report queued or delivered to a waiting thread through `await_canvasight_run`, but they must not display a sent state. Markdown preview remains a separate drawer command for reviewing generated content and must not be used as the only visual proof that a Run was sent.

Normal plugin Run delivery happens from the Codex native widget host bridge. Native open must bind the active task's explicit `CODEX_THREAD_ID` as `threadId` before rendering, so Chat / Plan / Goal preflight targets the widget-owning thread. The native widget should host the Canvasight app directly and use the project daemon only for APIs, avoiding a localhost iframe inside the widget. The injected Vite bundle must execute as an ES module; bootstrap feedback must remain loading until that module succeeds, clear when the canvas mounts, and show a concrete recovery error if the module fails. Widget CSP must include the current daemon's exact origin as well as localhost fallbacks so the direct widget app can fetch project data without `Failed to fetch`. Native open must present a widget result, not a visible localhost URL preview; daemon URLs belong only in widget metadata. If a native open tool is not visible, the agent must lazy-load Canvasight tools with `tool_search` before considering fallback. The bridge may use MCP Apps `ui/message` or the Codex/OpenAI compatibility `window.openai.sendFollowUpMessage`; both are native widget transports and must resolve before a sent state appears. A missing `hostCapabilities.message` declaration is diagnostic, not a disabled state, after MCP App initialization succeeds; actual `sendMessage` resolve/reject controls sent/failed. Browser URL and bare dev-server surfaces are fallback contexts; they require an explicit current-thread claim before queued Run recovery. They must not start or label app-server `turn/start` delivery as a successful node Run. It must label unbound, queued, browser-fallback-no-bridge, mode-preflight failed, bridge transport, bridge reason, and current-thread widget sent states distinctly. Diagnostics should stay compact and expose environment/delivery metadata without showing full Run Markdown.

Canvasight's thread-local MCP stdio process is a shim, not the long-lived project service. It should keep stdout reserved for JSON-RPC, write size-bounded lifecycle diagnostics to the Canvasight state directory, return JSON-RPC errors for tool failures, and exit cleanly when the host closes stdin or stdout. The project daemon remains separate and may outlive the shim, but concurrent shims must use one cross-process startup owner, stale daemon state from an older Canvasight version must be cleared, and an old daemon may not remove state owned by a newer process.
