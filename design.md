...
- Other files use one 360 px white surface with the existing Canvasight divider, radius, and node shadow. Center one large icon from the repository SVG registry; never show another gray inner card, filename, extension label, or byte size.
- Map common formats through the existing SVG registry: PDF/document, spreadsheet/data, presentation, archive, audio, video, code/text, database/dataset, notebook/ebook, design, font, 3D, installer, image, and a generic fallback. Do not introduce generated bitmap icons or external brand logos.
- Asset surfaces expose no editable title, description field, duplicate command, or Run control. Persisted title/description and file metadata remain readable for compatibility, accessibility, search, Markdown, and Run context but are not visible on the card.
