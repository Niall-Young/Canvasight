...
- Video is detected from MIME/extension without expanding the v2 schema. When the existing asset proxy can serve it, render the native video surface directly, do not autoplay, and show only necessary media controls. If the proxy cannot serve it safely, fall back to the existing video SVG instead of a broken player.
