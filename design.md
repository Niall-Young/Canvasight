...
Task attachments remain compact references. Dropping a file on a Task adds an attachment; the attachment overflow may promote it to an Asset Node without copying or deleting the managed file. Promotion removes only the inline reference, creates the Asset beside the Task, and adds an editable `Asset → Task` reference relationship as one undoable change.
...
- Video is detected from MIME/extension without expanding the v2 schema. When the existing asset proxy can serve it, render the video surface directly and never autoplay. Clicking the picture selects the Asset and never changes playback; a persistent bottom overlay toolbar owns play/pause through a localized, keyboard-reachable icon button, with playback state driven by the media's actual play, pause, and ended events. Do not expose native media controls whose picture-click behavior conflicts with canvas selection. If the proxy cannot serve the video safely, fall back to the existing video SVG instead of a broken player.
