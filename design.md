...
The `$` picker is rendered through a viewport-level Portal with fixed physical dimensions, so it never inherits XYFlow pan or zoom transforms. It opens beside the editing node and uses collision-aware placement to stay within the visible viewport rather than covering the node or drifting into an awkward canvas position. Fuzzy search must expose every matching item from the fully loaded Skill catalog; the picker may use an internally scrolling result list, but it must not hide matches behind a top-N cutoff. Composition input, keyboard and IME editing, Arrow keys, Enter/Tab selection, Escape dismissal, refresh, and an explicit manual-entry fallback must remain available without resizing the Page layout or replacing ordinary textarea editing.
