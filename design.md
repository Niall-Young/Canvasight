...
- Overflow is the only Asset header control and sits at `top: 12px; right: 12px`. It appears only while the Asset is hovered, focus-within, selected, or its menu is open, and contains only explicit Replace file and Delete management. Direct file opening may remain available from the content itself. Replacement uses the managed-asset import path, preserves node ID, position, Group membership, and Edges, and never deletes the previous managed file.
