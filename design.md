...
Normal plugin Run delivery happens from the Codex native widget host bridge. The native widget should host the Canvasight app directly and use the project daemon only for APIs, avoiding a localhost iframe inside the widget. Widget CSP must include the current daemon's exact origin as well as localhost fallbacks so the direct widget app can fetch project data without `Failed to fetch`. Native open must present a widget result, not a visible localhost URL preview; daemon URLs belong only in widget metadata. If a native open tool is not visible, the agent must lazy-load Canvasight tools with `tool_search` before considering fallback. The bridge may use MCP Apps `ui/message` or the Codex/OpenAI compatibility `window.openai.sendFollowUpMessage`; both are native widget transports and must resolve before a sent state appears. Browser URL and bare dev-server surfaces are fallback contexts; they require an explicit current-thread claim before queued Run recovery. They must not start or label app-server `turn/start` delivery as a successful node Run. It must label unbound, queued, browser-fallback-no-bridge, mode-preflight failed, bridge transport, bridge reason, and current-thread widget sent states distinctly. Diagnostics should stay compact and expose environment/delivery metadata without showing full Run Markdown.

Canvasight's thread-local MCP stdio process is a shim, not the long-lived project service. It should keep stdout reserved for JSON-RPC, write lifecycle diagnostics to the Canvasight state directory, return JSON-RPC errors for tool failures, and exit cleanly when the host closes stdin. The project daemon remains separate and may outlive the shim, but stale daemon state from an older Canvasight version must be cleared before opening a widget or reporting a dev server as healthy.
