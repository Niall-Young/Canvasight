...
- Task node: the primary executable object, with title, prompt/body content, connection affordances, run controls, and mode controls. It never exposes a file-upload control or accepts a new inline attachment.
- Asset node: one managed project file promoted to a first-class visible evidence object that connects to Tasks or other Assets and travels through Task/Group Run scope. Its purpose comes from placement, connection direction, Edge labels, and surrounding context; it is a file object, not an executable or text-editing surface.
...
- An empty Workflow Page gives one plain-language starting action: create the first Task. It explains the `write → connect when needed → run in the current Codex task` loop without hiding the canvas or adding a setup wizard.
...
Project History is a separate, project-level workspace reached through a compact `Workflow | Version history` segmented control. Workflow remains the default and keeps its existing Page model. Version history opens on `Feature map`, a plain-language view of what the project has delivered, what is being developed, and what has returned to the main path. `Git technical map` is the secondary factual view of the repository's real commit DAG. Neither perspective represents commits as runnable Task or Asset nodes or changes `.scatter` document revision, Page content, Workflow viewport, selection, drawer, or focus.

- Workflow and History each own independent viewport, selection, filters, overlay state, and keyboard focus. Switching restores the previous state of that workspace without fitting or scattering the other workspace. A long History graph must remain readable rather than merely fit inside the viewport: when a saved History viewport is below the readability threshold, entry performs a one-time rescue focus on the current task or current Git branch's latest active feature line (up to the four newest protected states). `View all history` remains an explicit secondary canvas action, and `Focus current progress` returns to the readable working context without changing graph data.
- The Feature map uses a left-to-right product tree. The protected project start occupies the first main-path position; integrated features continue along that path, while later or parallel work branches from its real dependency. A feature card summarizes the whole feature rather than only its latest checkpoint, and folds its restorable states into a count and expandable list. Search temporarily shows only matching feature cards in a compact non-overlapping row.
- The Git technical map uses a left-to-right Git DAG. Every returned real commit is a node, first-parent `main` is the stable horizontal reference, a branch label on an otherwise uninterrupted ancestor path stays linear, real divergence creates a side path, and merge commits visibly reconnect every parent. Uncommitted changes appear as a distinct working-tree state rather than a synthetic commit. The status bar states ahead/behind counts and the factual next Git action.
- Git technical-map entry focuses a readable window around HEAD and the working tree when the graph is long; `View full project` is the explicit zoomed-out overview. Canvasight-generated commits remain factual nodes but receive a concise auto-save label while their original Git subject stays available in details. Clicking a commit shows its hash, parents, refs, author, time, merge meaning, and directly linked Canvasight restore evidence.
- Enabling protection guarantees a real local `main`: reuse local `main`, otherwise create it from a remote `*/main`, local `master`, the current committed `HEAD`, or the initial protected baseline in that order. Never label the current feature branch as main merely because `main` is absent. The persistent header leads with protection health and the no-upload guarantee; exact local `main`, current branch, coverage start, and logical progress grouping remain available in the explicit reading/help panel.
- Every history node shows a short plain-language summary and explicit text for protection, source, confirmation, test, merge, and recovery completeness. Shape, label, icon, and badge must carry state; color is supporting information only.
- In `Feature map`, a large card means one understandable feature outcome, not one file change or Git commit. Automatic checkpoints stay folded inside the feature until the user needs recovery. Branch names, commit IDs, and changed files belong in collapsed technical evidence. A connection means a real feature dependency; it never turns chronological proximity into a dependency.
- Chat-only turns appear as a compact count on the connection between protected states. Expanding the count lists task and turn handles, but full chat remains in Codex. A location action sends an explicit native host-bridge request to open the original task; it must not use private deep links, DOM automation, or pretend that copied IDs are navigation.
- History nodes reuse the Workflow Task-card surface, radius, border, shadow, typography, and header rhythm. Color is limited to compact branch dots and status badges; warnings never become a full-card stripe or replace the node summary as the dominant information.
- Selecting a feature opens complete details directly; there is no intermediate quick-action panel. The graph stays full width and never reserves a permanent right inspector. Details lead with the feature outcome and current state, then the expandable checkpoint list. Technical evidence, legacy acceptance records, renaming, and grouping stay collapsed. Enabling protection, continuing from an old state, sending an integration request, or remote sync still require explicit user intent.
- A clearly labeled example-data preview may demonstrate branching, chat markers, merged and abandoned lines, incomplete coverage, quick actions, and the detail dialog before protection is enabled. It is session-local presentation state: it must not call Git/Codex write actions, mutate the real history index, or persist example nodes into the project.
- Current protection health is always visible; the exact coverage start is one click away in the reading/help panel. Incomplete recovery, external or mixed attribution, missing Git objects, scan failure, and unsynced metadata remain visible until resolved; the interface never says “complete history” when recording began later.
- Configured exclusion patterns alone do not make a node incomplete. Recovery warnings appear only when the capture observed relevant omitted local, sensitive, large, external-link, submodule, LFS, or truncated content. Generated dependencies and Canvasight's own internal history may remain informational. Warning copy must lead with the verified fact that the code snapshot is restorable when its Git object is available.
- Abandoned and merged feature lines become visually quiet and gray but remain expandable, searchable, selectable, and recoverable. Formal nodes are not deletable; editing a name, summary, or classification creates auditable metadata instead of rewriting the underlying event.
- Multiple saved states for the same feature collapse into one feature card. States with the same project tree are counted once in the product view, while the underlying recovery references remain intact. The badge states the retained checkpoint count, and details expand every distinct recoverable state without deleting history.
- `Integrate into project` does not perform a hidden merge. It sends the current Codex task a bounded request containing the feature outcome, branch, dependency, latest recovery point, and project path. Codex must inspect the actual state, run relevant checks, stop on conflicts or unfinished work, integrate only into local `main`, and never push or rewrite unrelated history. The request returns to idle after a bounded missing-receipt window. Legacy Agent-check and confirmation records remain readable as collapsed technical evidence but are not the MVP golden path.
- The portability panel is secondary to the graph. With no Git remote it exports and imports a bounded local JSON metadata manifest. With remotes it requires the user to select and authorize one remote before any synchronization. It states permanently that only summaries, layout, state, coverage, and Git references sync; chat and project code never do. Imported metadata with missing Git objects remains visible as an unavailable node and disables Continue.
- External changes are honest project activity, not fabricated Codex ownership. Stable dirty state seals after two minutes of silence, an external commit seals immediately, and the toolbar offers an explicit save-now action. Shared-checkout activity uses external or mixed source labels.
- Empty, loading, error, non-Git, no-task, and no-host-bridge states are first-class. A non-Git project first shows the scan scope and exclusions, then requests confirmation before local Git initialization. A failed protection operation preserves the current project and provides retry without showing a protected state.
- Narrow layouts preserve the graph as the primary surface, bound the card-anchored quick panel to the viewport, and keep complete details in a modal or bottom sheet. Keyboard users can switch workspaces, traverse nodes in time order, open quick actions and complete details, expand chat activity, and invoke location actions with visible focus. Screen-reader names include node kind, summary, state, source, and recovery completeness.
- The V1 History workspace has no chat input and no permanent right-side Codex panel. A future pure-canvas workspace may add chat, but it must not blur V1 navigation and provenance semantics.
...
- Valid legacy image attachments must render thumbnails from the actual image content. Loading and error states must use neutral, unmistakable, recoverable feedback and must never imitate image content or appear to be a real thumbnail.
- Parent and child connection handles should remain discoverable on the left and right edges.
- Clicking either connection-handle plus opens the same compact creation menu for Task, file, or media. Task selection creates immediately; file and media selection each open a single-file picker and create the connected Asset only after a valid selection saves. Media means images (including SVG) and video, while ordinary files use the file entry. Dragging a handle to an existing node still connects directly; dropping on blank canvas opens the same menu at the drop point and creates nothing until a type is chosen.
...
Task Nodes do not expose file-upload controls and never accept new inline attachments. File selection, drop, and file/image paste create first-class Asset Nodes even when the pointer or caret is over a Task. Existing inline Task attachments remain visible, runnable, exportable, removable, and explicitly promotable for backward compatibility; opening or saving an old document must never delete or automatically migrate them. Promotion keeps its explicit one-step migration behavior: reuse the managed file, remove only the inline reference, create the Asset beside the Task, and add a `Task → Asset` relationship without copying or deleting project data. This direction lets one Task fan out to multiple files and makes Task Flow Run include every promoted Asset while preserving the single-predecessor Edge invariant.
...
- Use a stable 360 px width. The managed content itself is the Asset boundary; do not nest a file card inside a node shell or add a persistent header/footer.
- Images use the native asset proxy and their intrinsic ratio, constrained only for extreme dimensions. The image itself is the visible node surface: no outer shell, padding, gray preview well, filename, type, size, or footer. Loading/error states may reserve a neutral placeholder without imitating content.
- Managed `.svg` files are first-class Image Assets even when an importer supplies an empty or generic MIME type. They use the same direct media surface as raster images and never fall back to the ordinary file card or format-icon layout merely because the image is vector content. A non-`.svg` file that only claims `image/svg+xml` must fail closed instead of being guessed as SVG.
- Treat SVG as untrusted passive image content. Sanitize the preview before display, then render it only through an `<img>` element; never inject SVG markup into the application DOM or render it with `dangerouslySetInnerHTML`, `object`, `embed`, `iframe`, or equivalent active-document surfaces. Strip executable elements, event handlers, `foreignObject`, external references, and unsafe CSS URLs while preserving safe vector structure such as `viewBox`, gradients, local fragment references, and inline presentation styles. Browser/dev and native-widget previews must follow the same rule.
- Video is detected from MIME/extension without expanding the v2 schema. When the existing asset proxy can serve it, render the video surface directly with the browser's complete native controls and never autoplay. Clicking the picture selects the Asset and never changes playback; a transparent picture-only selection layer must stop above the native control strip so play/pause, progress, time, volume, picture-in-picture, and fullscreen remain browser-owned and unobstructed. Do not replace that native toolbar with a custom playback button. If the proxy cannot serve the video safely, fall back to the existing video SVG instead of a broken player.
- Other files use one compact 360 px white surface with the existing Canvasight divider, radius, and node shadow. Use a 112 px minimum height and lay out a 48 px format icon on the left with a two-line filename plus `FORMAT · SIZE` summary on the right. The content uses `16px 56px 16px 16px`: Task-aligned padding plus a permanent right-side safe area for the hover More button. Never add a gray inner card.
- For non-image files, use the supplied SVG format set through the existing registry: PDF, Markdown, presentation, CSV, spreadsheet, document, and common code formats. Any format outside that explicit mapping uses the supplied unknown-file SVG. Source `.svg` images are media, not format icons. Do not substitute generated bitmap icons, external brand logos, or unrelated repository icons.
- Asset surfaces expose no editable title, description field, duplicate command, or Run control. Image/video filename and size remain visually hidden; ordinary file Assets show the managed filename and lightweight format/size summary while keeping persisted title/description out of the card.
...
- Selected image and video Assets show a one-pixel `--color-border-focus` overlay ring inside the direct media boundary. The ring must not participate in layout or change the media rect, node bounds, Handle centers, or Edge endpoints; keyboard `focus-visible` remains a separate outer focus treatment.
- Do not show or edit Asset classification on the canvas. The persisted v2 `role` field remains readable only as legacy schema compatibility; it must not drive visible UI, accessible naming, or Run Markdown. Edge direction, labels, and context carry the relationship meaning.
- Asset overflow sits at `top: 12px; right: 12px` and appears only while the Asset is hovered, focus-within, selected, or its menu is open. It contains only explicit Replace file and Delete management. Direct file opening may remain available from the content itself. Replacement uses the managed-asset import path, preserves node ID, position, Group membership, and Edges, and never deletes the previous managed file.
- Product-design generation produces exactly three independent UI directions unless the user requests another count, then stops so the user can connect the preferred image into the existing flow. These images are ordinary Asset Nodes—node-shaped attachments, not a new selection model. Task Flow Run follows the existing graph and includes reachable Assets; candidates outside that Task's flow are unrelated. Do not encode selection with the legacy Asset `role` field.
- Empty-canvas and Task drops create Asset nodes at the pointer; a Task inside a Group creates sibling Asset members in that Group. Dropping on an existing Asset never silently replaces its file.

File import cannot be drag-only. The keyboard-reachable canvas Asset picker, canvas/Task/Group drop, and file/image paste all create Asset nodes; Task targeting never changes that import into an inline attachment. Image alternative text and the Asset accessible name use the original filename even when it is visually hidden. File loading uses polite status text; failure uses an explicit message plus Retry or Open rather than color or an image-like placeholder. Replace, direct Open, and Delete remain keyboard reachable, with localized accessible names and visible focus. The content surface remains the primary drag target; only controls, handles, menus, and native video controls are `nodrag`.
...
- `Cmd/Ctrl+G` groups selected Task/Asset nodes; `Cmd/Ctrl+Shift+G` releases selected members or every member of a selected Group.
- Creating a Group fits selected members with consistent padding. Adding members may grow the Group but must not shrink a user-resized frame. Explicit Fit to contents may shrink it.
- Deleting a Group releases members in place and preserves their semantic edges. Empty Groups remain valid categories, but cannot Run.
- Resize affordances appear only while the Group is selected. An empty expanded Group shows a compact `Drag nodes here` instruction.
...
The node body is a toolbarless, Markdown-triggered WYSIWYG editor that remains part of the task node rather than becoming a document surface. Markdown shortcuts may transform as the user types, but Markdown `string` remains the sole persisted, template, export, and Run representation; the editor must not introduce HTML or editor-JSON state into the document contract.

- V1 rich text covers headings levels 1–3, bold, italic, strikethrough, bullet and ordered lists, blockquotes, inline code, fenced code blocks, and links. Task bodies do not expose interactive task-list nodes or checkboxes: typed, pasted, and legacy `- [ ]` / `- [x]` markers remain non-interactive ordinary list text and must round-trip without losing their marker state. Framework-question multi-select controls and the workspace Task list drawer are separate product surfaces and remain unchanged. Tables, inline images, text color, font selection, and alignment are outside this editor contract; file/image paste creates Asset Nodes, while legacy attachments may remain in the compact compatibility area below the body.
- AI graph writes use the same Markdown-string contract. They must not author editor JSON, raw HTML, tables, inline Markdown media, data URLs, or local absolute paths as substitutes for Asset Nodes. Historical unsupported Markdown remains opaque and must be preserved when its Task body is not being edited.
- Read and edit states use the same document structure, typography, wrapping, and spacing so entering or leaving edit mode does not reflow the node. The body has no toolbar, floating menu, nested card, background, border, shadow, or editor-library default chrome; the task node remains the only visual container.
- Typography and states use the existing `app.css` semantic tokens and compact node-body scale. Headings, paragraphs, lists, quotes, code, links, placeholders, focus, selection, and light/dark themes must not introduce hard-coded editor colors, type scales, radii, or spacing.
- Inline-code semantics are bounded by Markdown backticks in source and serialization; straight or curly single quotation marks remain ordinary punctuation. In WYSIWYG editing, pressing Enter from within or at the end of an inline-code span creates an unmarked next block, and pressing Space at the end of a span inserts an unmarked space so following text is ordinary; spaces inside the span remain code content. Users should not need to toggle code off manually. Inline code and fenced code blocks use the canvas-background token in both themes so they remain visibly distinct from the Task surface. Inline code reserves a transparent one-pixel border in light mode and reveals the neutral divider border in dark mode without changing its box size. Fenced blocks retain their own neutral divider border, and their nested `code` clears the inline border and stays transparent to avoid a second layer. Fenced code blocks use the existing `space-10` token to separate them from adjacent top-level body content while preserving the node body's compact rhythm.
- A selected node may enter body editing on a subsequent body interaction. Text selection, IME composition, ordinary editing shortcuts, and content scrolling must coexist with XYFlow: the editor participates in the existing `nodrag` and `nowheel` behavior, and body-size changes notify XYFlow from a `ResizeObserver` so handles and edges remain aligned without layout jumps.
- Paste accepts only supported text, Markdown, and sanitized rich-text structure. Scripts, event attributes, unsafe URL protocols, and unsupported embedded media are discarded; safe links remain visibly distinguishable and keyboard accessible.
- `$Skill` discovery is anchored to the current contenteditable selection and editor caret. Inserting a result produces ordinary visible `$skill-name` text at that range, preserving the same composition, keyboard, Portal, and accessibility behavior as manual token entry.

## Codex Run

Run is a single Chat submission action on Task and Group surfaces. It uses a play icon and sends the selected executable scope as a normal Codex conversation prompt; there is no Plan or Goal mode selector. Task Flow Run follows persisted semantic edges and carries downstream Asset evidence. Assets are not executable and never expose their own Run. Group Run is deliberately bounded: it includes the Group title, description, direct members, member assets, and internal edges only, and never traverses to nodes outside the Group.
...
Review and export use Groups as chapters. Direct Group members become ordered subsections; disconnected members use stable canvas reading order by absolute X, then Y, then ID. Ungrouped Task/Asset nodes appear under one localized Ungrouped chapter when any exist. Asset description, managed-file reference, Edge direction/labels, and Group structure remain explicit so the receiving AI can understand why a file matters; legacy Asset role is not emitted.
...
Canvasight supports Codex or other AI agents writing `.scatter/scatter.json` to create pages, Task/Asset/Group nodes, and edges. New file content is represented by managed Asset Nodes; Task attachment metadata is a legacy read-compatibility surface, not an AI creation path. AI-generated canvas content should feel like normal editable canvas state, not a separate imported artifact.

Graph Context exposes only lightweight legacy-attachment handles (`id`, inferred kind, original name, project-relative path, MIME, and size), never absolute paths or file URLs. A context-bound `promote-attachment` mutation atomically removes one legacy Task reference, creates a same-Group Asset, and adds `Task → Asset`. Legacy template attachments are copied into the current project's managed asset directory and expanded into downstream Asset Nodes; failed validation or copying must leave both the document and asset directory unchanged. Persisted Asset `role` remains read-compatible but is deprecated for AI intent.
...
- AI-created nodes must use the same visual language, spacing, handles, Asset file objects, and mode controls as manually created nodes.
...
The `$` picker is a compact combobox anchored to the active rich-text editor caret and contenteditable selection, not a node-side panel. It is rendered through a viewport-level Portal with fixed physical dimensions, so it never inherits XYFlow pan or zoom transforms. Its normal state has no title bar or refresh action, is approximately 288 px wide, shows only four to five result rows, and scrolls internally for additional matches. Placement prefers the available space above or below the caret, falls back to the left or right only when vertical space is insufficient, and clamps to the visible viewport without covering the caret's current text line. Fuzzy search must expose every matching item from the fully loaded Skill catalog; scrolling may limit what is visible at once, but a top-N cutoff must not hide matches. Ordinary contenteditable composition, IME behavior, Arrow keys, Enter/Tab selection, Escape dismissal, explicit manual entry, and accessible combobox/listbox ARIA semantics must remain available without resizing the Page layout or replacing ordinary rich-text editing.
...
- When multiple AI capabilities produce canvas content, their output should share the same Task/Asset/Group, Edge, Page, and drawer patterns; they must not reintroduce new inline Task attachments.
...
- Group collapse uses `aria-expanded`. Group roots are named by their title, member count, and asset count; Fit, Run, collapse/expand, overflow, Asset replacement/direct Open, Group, and Ungroup remain reachable without pointer precision.
