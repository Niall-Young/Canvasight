...
- Other files use one compact 360 px white surface with the existing Canvasight divider, radius, and node shadow. Lay out a 48 px format icon on the left and a two-line filename plus `FORMAT · SIZE` summary on the right. Use the same 16 px side and bottom padding as a plain Task card, reserving only the upper control row; never add a gray inner card.
...
- Role (`input`, `reference`, `option`, or `output`) is metadata presented through a neutral compact dropdown that stays visible in the upper-left. It shows the current value and opens an accessible single-choice menu; never encode roles through four competing colors.
- Overflow appears in the upper-right only while the Asset is hovered, focus-within, selected, or its menu is open. Classification is not duplicated there; overflow contains only explicit Replace file and Delete management. Direct file opening may remain available from the content itself. Replacement uses the managed-asset import path, preserves node ID, position, Group membership, Edges, and role, and never deletes the previous managed file.
...
File import cannot be drag-only. The existing keyboard-reachable file picker must create Asset nodes when no Task is the explicit target, while an explicit Task target continues to receive attachments. Image alternative text and the Asset accessible name use the original filename even though it is visually hidden. File loading uses polite status text; failure uses an explicit message plus Retry or Open rather than color or an image-like placeholder. Classification, Replace, direct Open, and Delete remain keyboard reachable, with localized accessible names and visible focus. The content surface remains the primary drag target; only controls, handles, menus, and native video controls are `nodrag`.
...
Review and export use Groups as chapters. Direct Group members become ordered subsections; disconnected members use stable canvas reading order by absolute X, then Y, then ID. Ungrouped Task/Asset nodes appear under one localized Ungrouped chapter when any exist. Asset role, description, managed-file reference, and Edge labels remain explicit so the receiving AI can understand why a file matters.
