...
- Asset node: one managed project file promoted to a first-class visible evidence object that connects to Tasks or other Assets and travels through Run scope. Its purpose comes from placement, connection direction, Edge labels, and surrounding context. It is not a text-editing surface, but it may start a downstream Run so a user can choose one visual or file reference and send only that path to Codex.
...
- Asset surfaces expose no editable title, description field, or duplicate command. Image/video filename and size remain visually hidden; ordinary file Assets show the managed filename and lightweight format/size summary while keeping persisted title/description out of the card. A compact play control may start the Asset and its downstream flow, allowing one generated UI reference to become the primary visual target without sending sibling options.
...
- Asset Run and overflow form one compact control cluster at `top: 12px; right: 12px`. They appear only while the Asset is hovered, focus-within, selected, or its menu is open. Run uses the shared play icon; overflow contains only explicit Replace file and Delete management. Direct file opening may remain available from the content itself. Replacement uses the managed-asset import path, preserves node ID, position, Group membership, and Edges, and never deletes the previous managed file.
...
Run is a single Chat submission action on Task, Asset, and Group surfaces. It uses a play icon and sends the selected object as a normal Codex conversation prompt; there is no Plan or Goal mode selector. Task and Asset Flow Run follow persisted semantic edges. An Asset-started Run treats that file as the user's selected primary reference, includes its exact managed image path when applicable, and includes downstream implementation instructions without pulling in unselected sibling Assets. Group Run is deliberately bounded: it includes the Group title, description, direct members, member assets, and internal edges only, and never traverses to nodes outside the Group.
...
- Group collapse uses `aria-expanded`. Group roots are named by their title, member count, and asset count; Fit, Run, collapse/expand, overflow, Asset Run/replacement/direct Open, Group, and Ungroup remain reachable without pointer precision.
