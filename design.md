...
- V1 rich text covers headings levels 1–3, bold, italic, strikethrough, bullet and ordered lists, task lists, blockquotes, inline code, fenced code blocks, and links. Tables, inline images, text color, font selection, and alignment are outside this editor contract; attachments remain in the existing compact attachment area below the body.
...
- Typography and states use the existing `app.css` semantic tokens and compact node-body scale. Headings, paragraphs, lists, quotes, code, links, placeholders, task controls, focus, selection, and light/dark themes must not introduce hard-coded editor colors, type scales, radii, or spacing.
